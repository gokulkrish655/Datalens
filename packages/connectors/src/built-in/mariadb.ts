...
export const mariadbConnector: IDbConnector = {
  ...
  async introspectTables(config: ConnectionConfig): Promise<SchemaTableInfo[]> {
    ...
    try {
      ...
      return rows.map((row: any) => ({
        tableName: (row as any).TABLE_NAME ?? (row as any).table_name,
        schemaName: schema,
        displayName: (row as any).TABLE_NAME ?? (row as any).table_name,
      }));
    } finally {
      await connection.end();
    }
  ...
  async introspectColumns(
    ...
    try {
      ...
      return rows.map((row: any) => ({
        tableName: (row as any).TABLE_NAME ?? (row as any).table_name,
        schemaName: schema,
        columnName: (row as any).COLUMN_NAME ?? (row as any).column_name,
        displayName: (row as any).COLUMN_NAME ?? (row as any).column_name,
        dataType: (row as any).DATA_TYPE ?? (row as any).data_type,
  ...
  async executeQuery(
    ...
    try {
      const rows = (await connection.query(sql)) as Array<Record<string, unknown>>;
      return {
